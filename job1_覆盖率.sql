    ...
    (
        ...
        right join 
            (
                ...
                where 
                    p_date = '20210721' and user_id > 0 and is_today_active = 1
                group by 
                    user_id, 
                    is_reg_user,
                    product
...
select 
    ...
    dau.product,
    count(1) as gid_cnt 
    ,sum(if( ser.gid is not null,1,0)) as serv_gid_cnt  
    , 'gid_dau' as product_type
from 
--转换
    (
    select left_value as gid
    from kskdp.fvs_ffs_ue_idmp_all_id_pair_rel_all
    where p_date = '20210620' and left_type = 'GLOBAL_ID' and left_value is not null and right_value is not null
    group by left_value
    ) as ser
right join 
    (
    ...
    from
        (
        select global_id,product,'np' as p_tag
        from kskdp.fvs_ffs_ue_device_product_device_global_mapping_1d
        where p_date = '20210621' 
        and product not in ("KUAISHOU","NEBULA","KUAISHOU_LIVE_MATE","THANOS","KUAISHOU_ANTMAN","KUAISHOU_LANDSCAPE","KS_TV","GAME_ZONE","KWAI_LITE")
        ...
        group by global_id,product
        union all
        select
          global_id
         ,default.product_clean(product,'20210621') product
         ,'ks' as p_tag
     ...
     and global_id is not null and global_id > '0000000008f0d180' and (length(trim(global_id)) = 16 or length(DeGidUDF(trim(global_id)) ) = 16)
     group by global_id,default.product_clean(product,'20210621')
        ) dau
    group by global_id,product,p_tag
    ) dau
on dau.global_id = ser.gid
group by 
dau.product,dau.p_tag
order by
dau.p_tag,
gid_cnt desc
...
select 
    ...
    count(case when ser.left_value is not null then left_value end) as serv_imei_cnt ,
    round(count(case when ser.left_value is not null then left_value end)*100.0/count(dau.imei),4) as rate,
...
--转换
    ...
    select left_value 
    from kskdp.fvs_ffs_ue_idmp_all_id_pair_rel_all
    where p_date = '20210620' and left_type = 'IMEI' and left_value is not null and right_value is not null
    group by left_value
    ) as ser
right join 
    (
    select imei,product,kpn
    from
        (
        select imei,product,'ks' as kpn
        from kscdm.dwd_ks_tfc_stat_device_hi
        where p_date = '20210621' and imei is not null and length(imei) in (15, 14)
        group by imei,product
        
        union all
        
        select imei,product,'np' as kpn
        from npcdm.dwd_pub_tfc_stat_device_hi
        where p_date = '20210621' and imei is not null and length(imei) in (15, 14)
        group by imei,product
        ) as dau
    group by imei,product,kpn
    ) dau
on dau.imei = ser.left_value
group by
dau.product,dau.kpn
order by
dau.kpn,imei_cnt desc limit 100000
...
select
    ...
    count(case when ser.left_value is not null then left_value end) as serv_idfa_cnt ,
...
--转换
    ...
    select left_value
    from kskdp.fvs_ffs_ue_idmp_all_id_pair_rel_all
    where p_date = '20210620' and left_type = 'IDFA' and left_value is not null and right_value is not null
    group by left_value
    ) as ser
right join
    (
    select idfa,product,kpn
    from
        (
        select idfa,product,'ks' as kpn
        from kscdm.dwd_ks_tfc_stat_device_hi
        where p_date = '20210621' and idfa is not null and idfa  <> '00000000-0000-0000-0000-000000000000' and length(idfa) = 36
        group by idfa,product

        union all

        select idfa,product,'np' as kpn
        from npcdm.dwd_pub_tfc_stat_device_hi
        where p_date = '20210621' and idfa is not null and idfa  <> '00000000-0000-0000-0000-000000000000' and length(idfa) = 36
        group by idfa,product
        ) as dau
    group by idfa,product,kpn
    ) dau
on dau.idfa = ser.left_value
group by
dau.product,dau.kpn
order by
dau.kpn,idfa_cnt desc limit 100000
...
select 
    ...
    ,count(case when ser.left_value is not null then left_value end) as serv_phone_cnt 
...
--转换
    ...
    select left_value 
    from kskdp.fvs_ffs_ue_idmp_all_id_pair_rel_all
    where p_date = '20210620' and left_type = 'PHONE' and left_value is not null and right_value is not null
    group by left_value
    ) as ser
right join 
    (
    ...
    from
        (
        select phone,product,'ks' as p_tag
        ...
        select phone,product,'np' as p_tag
        from npcdm.dim_pub_user_all
        where p_date = '20210621' and phone is not null
        group by phone,product
        ) dau
    group by phone,product,p_tag
    ) dau
on dau.phone = ser.left_value
group by
p_tag,
product
order by p_tag,phone_cnt desc
